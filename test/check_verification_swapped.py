...
def parse_g2_swapped(hex_str):
    # Assume hex_str is from to_bytes_eth (Im, Re).
    # But we want to swap interpretation: First 64 is Re, Next is Im.
    x_c0 = int(hex_str[:64], 16) # Re
    x_c1 = int(hex_str[64:128], 16) # Im
    y_c0 = int(hex_str[128:192], 16) # Re
    y_c1 = int(hex_str[192:], 16) # Im
    return (FQ2([x_c0, x_c1]), FQ2([y_c0, y_c1]))
...
A = parse_g1(A_hex)
B = parse_g2(B_hex) # B is definitely correct with standard parse
C = parse_g1(C_hex)
...
beta_neg = parse_g2_swapped(beta_neg_hex)
gamma_neg = parse_g2_swapped(gamma_neg_hex)
delta_neg = parse_g2_swapped(delta_neg_hex)
...
print("Computing Miller Loops (Swapped VK)...")
