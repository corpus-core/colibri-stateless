...
inline void expHardPartBN(Fp12& y, const Fp12& x)
{
	...
	Fp12 a2, a3;
	pow_z(b, x); // x^z
    std::cout << "FE x^z: " << b << std::endl;
	fasterSqr(b, b); // x^2z
    std::cout << "FE x^2z: " << b << std::endl;
	fasterSqr(a, b); // x^4z
	a *= b; // x^6z
	pow_z(a2, a); // x^(6z^2)
	a *= a2;
	...
	pow_z(a3, a3); // x^(12z^3)
	a *= a3;
    std::cout << "FE a (part1): " << a << std::endl;
	Fp12::unitaryInv(b, b);
	b *= a;
	a2 *= a;
	Fp12::Frobenius2(a, a);
	a *= a2;
	a *= x;
    std::cout << "FE a (part2): " << a << std::endl;
	Fp12::unitaryInv(y, x);
	y *= b;
	Fp12::Frobenius(b, b);
	a *= b;
	Fp12::Frobenius3(y, y);
	y *= a;
    std::cout << "FE y (final): " << y << std::endl;
#else
	Fp12 t1, t2, t3;
	Fp12::Frobenius(t1, x);
