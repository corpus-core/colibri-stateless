...
| Log Level | `-l, --log_level` | `LOG_LEVEL` | `0` | Log verbosity (0-4). Use `4` for detailed debug (incl. head polling) |
...
These knobs tune the heuristic load balancing (AIMD) and the RPC head polling used to estimate block availability and reduce retries:

| Variable | Default | Description |
|----------|---------|-------------|
| `C4_MAX_CONCURRENCY_DEFAULT` | `8` | Initial per-server concurrency limit |
| `C4_MAX_CONCURRENCY_CAP` | `64` | Upper bound for dynamic concurrency |
| `C4_LATENCY_TARGET_MS` | `200` | Target EWMA latency for additive increase |
| `C4_CONC_COOLDOWN_MS` | `30000` | Cooldown between AIMD adjustments |
| `C4_OVERFLOW_SLOTS` | `1` | Allowed overflow slots when all servers saturated |
| `C4_SATURATION_WAIT_MS` | `100` | Wait before using overflow on full saturation |
| `C4_METHOD_STATS_HALF_LIFE_SEC` | `60` | Half-life for method success/not-found EWMA |
| `C4_RPC_HEAD_POLL_ENABLED` | `1` | Enable `eth_blockNumber` polling per RPC server |
| `C4_RPC_HEAD_POLL_INTERVAL_MS` | `6000` | Poll interval (ms). 6s recommended |

With `LOG_LEVEL=4`, the server logs head polling results (per RPC server) to help diagnose routing decisions.
