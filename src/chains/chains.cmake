...
function(generate_verifiers_header)
    ...
    foreach(prop ${VERIFIER_PROPERTIES})
        ...
        file(APPEND ${VERIFIERS_H} "method_type_t ${method_type}(chain_id_t chain_id, char* method, json_t params, verify_flags_t flags);\n\n")
    endforeach()
    ...
    # Add request_container function
    file(APPEND ${VERIFIERS_H} "method_type_t c4_get_method_type(chain_id_t chain_id, char* method, json_t params, verify_flags_t flags) {\n")
    file(APPEND ${VERIFIERS_H} "  method_type_t type = METHOD_UNDEFINED;\n")
    foreach(prop ${VERIFIER_PROPERTIES})
        string(REPLACE ":" ";" parts "${prop}")
        list(GET parts 3 method_type)
        file(APPEND ${VERIFIERS_H} "  if (!type) type = ${method_type}(chain_id, method, params, flags);\n")
    endforeach()
    ...
    # Add init_rpc_ctx hook declarations for modules that provide one
    foreach(prop ${VERIFIER_PROPERTIES})
        string(REPLACE ":" ";" parts "${prop}")
        list(LENGTH parts count)
        if(count GREATER 5)
            list(GET parts 5 init_rpc_ctx)
            if(NOT "${init_rpc_ctx}" STREQUAL "")
                file(APPEND ${VERIFIERS_H} "void ${init_rpc_ctx}(c4_init_ctx_t* ctx);\n")
            endif()
        endif()
    endforeach()

    # Add c4_init_rpc_ctx dispatcher (no-op if no module registered a hook)
    ...
    foreach(prop ${VERIFIER_PROPERTIES})
        ...
        if(count GREATER 5)
            list(GET parts 5 init_rpc_ctx)
            if(NOT "${init_rpc_ctx}" STREQUAL "")
                file(APPEND ${VERIFIERS_H} "  ${init_rpc_ctx}(ctx);\n")
            endif()
        endif()
    endforeach()
    file(APPEND ${VERIFIERS_H} "}\n\n")
