...
pub fn _root_hash(keys: &[u8], out: &mut [u8; 32], gindex: u32) {
    // Recursive implementation matching the C logic and SSZ spec.
    
    if gindex >= 512 {
        // LEAF NODE Case (The actual keys)
        // Each leaf covers one key (48 bytes).
        // SSZ requires chunks to be 32 bytes. 48 bytes keys are split into two chunks:
        // Chunk 1: First 32 bytes
        // Chunk 2: Last 16 bytes + 16 bytes padding (zero)
        
        let offset = (gindex - 512) as usize * 48;
        if offset + 48 > keys.len() {
             // Boundary check, should not happen if keys is correct length
             return;
        }
        let mut left = [0u8; 32];
        let mut right = [0u8; 32];
        
        // Copy first 32 bytes of the BLS key
        left.copy_from_slice(&keys[offset..offset+32]);
        
        // Copy remaining 16 bytes of the BLS key to the start of right chunk
        // The rest of 'right' remains 0 (implicit padding)
        right[0..16].copy_from_slice(&keys[offset+32..offset+48]);
        
        *out = sha256_merkle(&left, &right);
    } else {
        // INTERNAL NODE Case
        // Recursively calculate hash of left and right children
        let mut left = [0u8; 32];
        let mut right = [0u8; 32];
...
pub fn verify_slot(slot: &[u8; 8], proposer: &[u8; 8], proof: &[u8; 32]) -> bool {
    ...
    // We copy the 8 bytes into the start of the 32-byte buffer.
    // The remaining 24 bytes are zeros (padding).
