...
pub fn _root_hash(keys: &[u8], out: &mut [u8; 32], gindex: u32) {
    // Recursive implementation matching C _root_hash
    // C logic:
    // if (gindex >= 512) {
    ...
    if gindex >= 512 {
        ...
        if offset + 48 > keys.len() {
             // Should not happen if keys is 512*48
             return;
        }
        let mut left = [0u8; 32];
        ...
        // remaining 16 bytes of right are 0 by initialization
...
pub fn verify_slot(slot: &[u8; 8], proposer: &[u8; 8], proof: &[u8; 32]) -> bool {
    ...
    // C: memcpy(slot_hash, slot, 8) -> pad with 0s?
    // The C code initializes to {0} then memcpy 8 bytes. So yes, padded with zeros.
