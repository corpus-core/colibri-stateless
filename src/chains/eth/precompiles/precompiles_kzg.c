...
static bool load_g2_tau_from_file(const char* path) {
  ...
  if (num_g1 <= 0) { fclose(f); return false; }
  // read NUM_G2
  if (!fgets(line, sizeof(line), f)) { fclose(f); return false; }
  long num_g2 = strtol(line, NULL, 10);
  if (num_g2 < 2) { fclose(f); return false; }
  // skip G1 lines
  ...
  uint8_t comp[96];
  if (!hex_to_bytes_stack(line, comp, sizeof(comp))) { fclose(f); return false; }
  fclose(f);
