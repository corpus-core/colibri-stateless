...
// Curve parameter B = 3
static const uint8_t BN254_B[] C4_UNUSED = {
...
static void fp12_frob(bn254_fp12_t* r, const bn254_fp12_t* a) {
  ...
  memset(&one, 0, sizeof(bn254_fp_t));
  one.bytes[31] = 1; // BE 1
  intx_sub(&p_minus_1, &p_val, &one);
  bn254_fp_t exp1, exp2, three, six;
  memset(&three, 0, sizeof(bn254_fp_t));
  three.bytes[31] = 3;
  memset(&six, 0, sizeof(bn254_fp_t));
  six.bytes[31] = 6;
  ...
  // c00
  res.c0.c0 = a->c0.c0;
  fp_neg(&res.c0.c0.c1, &res.c0.c0.c1);
  // c01
  t = a->c0.c1;
  fp_neg(&t.c1, &t.c1);
  fp2_mul(&res.c0.c1, &t, &xi_p_3);
  // c02
  t = a->c0.c2;
  fp_neg(&t.c1, &t.c1);
  fp2_mul(&res.c0.c2, &t, &xi_p_3_2);
  // c10
  t = a->c1.c0;
  fp_neg(&t.c1, &t.c1);
  fp2_mul(&res.c1.c0, &t, &xi_p_6);
  // c11
  t = a->c1.c1;
  fp_neg(&t.c1, &t.c1);
  fp2_mul(&tmp, &xi_p_6, &xi_p_3);
  fp2_mul(&res.c1.c1, &t, &tmp);
  // c12
  t = a->c1.c2;
  fp_neg(&t.c1, &t.c1);
  fp2_mul(&tmp, &xi_p_6, &xi_p_3_2);
  fp2_mul(&res.c1.c2, &t, &tmp);
...
bool bn254_fp12_is_one(const bn254_fp12_t* a) {
  ...
  for (int i = 0; i < 31; i++)
    if (a->c0.c0.c0.bytes[i] != 0) return false;
...
static void g1_normalize(bn254_g1_t* r, const bn254_g1_t* p) {
  ...
  if (p->z.bytes[31] == 1) {
    ...
    for (int i = 0; i < 31; i++)
      if (p->z.bytes[i] != 0) others_zero = false;
    if (others_zero) {
      *r = *p;
      return;
    }
...
static void line_func_dbl(bn254_fp12_t* f, bn254_g2_t* Q, const bn254_g1_t* P) {
  ...
  memset(&px_fp2, 0, sizeof(bn254_fp2_t));
  px_fp2.c0 = P->x;
  memset(&py_fp2, 0, sizeof(bn254_fp2_t));
  py_fp2.c0 = P->y;
...
void bn254_miller_loop(bn254_fp12_t* res, const bn254_g1_t* P_in, const bn254_g2_t* Q) {
  ...
  memset(&xi, 0, sizeof(bn254_fp2_t));
  xi.c0.bytes[31] = 9;
  xi.c1.bytes[31] = 1; // BE 9, BE 1
  bn254_fp_t p_minus_1, one, three, two, exp1, exp2;
  memset(&one, 0, sizeof(bn254_fp_t));
  one.bytes[31] = 1;
  memset(&three, 0, sizeof(bn254_fp_t));
  three.bytes[31] = 3;
  memset(&two, 0, sizeof(bn254_fp_t));
  two.bytes[31] = 2;
  ...
  bn254_g2_t Q1, Q2;
  Q1.x = Q->x;
  fp_neg(&Q1.x.c1, &Q1.x.c1);
  fp2_mul(&Q1.x, &Q1.x, &xi_p_3);
  Q1.y = Q->y;
  fp_neg(&Q1.y.c1, &Q1.y.c1);
  fp2_mul(&Q1.y, &Q1.y, &xi_p_2);
  memset(&Q1.z, 0, sizeof(bn254_fp2_t));
  Q1.z.c0.bytes[31] = 1;
  Q2.x              = Q1.x;
  fp_neg(&Q2.x.c1, &Q2.x.c1);
  fp2_mul(&Q2.x, &Q2.x, &xi_p_3);
  Q2.y = Q1.y;
  fp_neg(&Q2.y.c1, &Q2.y.c1);
  fp2_mul(&Q2.y, &Q2.y, &xi_p_2);
  memset(&Q2.z, 0, sizeof(bn254_fp2_t));
  Q2.z.c0.bytes[31] = 1;
