...
static void c4_period_prover_spawn_host(uint64_t target_period, uint64_t prev_period) {
  ...
  // Extend environment for rust host (do not log secrets).
  extern char** environ;
  int           env_count = 0;
  ...
  char* env_balance_file = bprintf(NULL, "SP1_BALANCE_FILE=%s/sp1_balance.txt", eth_config.period_store);
