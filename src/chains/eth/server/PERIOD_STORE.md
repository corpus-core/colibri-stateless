...
- The period store enables:
  ...
  - **Light client caching** (LCU per period)
...
- ZK-related files (produced by `scripts/run_zk_proof.sh`):
  ...
  - `zk_proof.ssz`: Packs `zk_proof_g16.bin` into the SSZ container used for sync-data delivery (`ZKSyncDataV6`, see `C4_ETH_REQUEST_SYNCDATA_UNION[2]` in `src/chains/eth/ssz/verify_types.c`). Carries the `headerChain` variant (embedded `checkpoint` is the epoch boundary right after the period's last block, anchored via a chain of headers up to the signed sync committee). The prover replaces this with a freshly-fetched LightClientBootstrap-derived `checkpoint` at request-assembly time when no `witness_keys` are configured -- see `c4_get_syncdata_proof` in `prover/historic_proof.c`. The witness-key path keeps `headerChain` because witness BLS signatures vouch for the signed header directly.
