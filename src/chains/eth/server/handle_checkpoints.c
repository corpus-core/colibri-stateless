...
static bool append_signable_period(uint64_t period, uint64_t* periods, file_data_t* files, int* num) {
  if (!c4_ps_file_exists(period, C4_PS_ZK_PROOF_SSZ)) return false;
  periods[*num]    = period;
  files[*num].path = bprintf(NULL, "%s/%l/" C4_PS_ZK_PROOF_SSZ, eth_config.period_store, period);
  (*num)++;
  return true;
}
...
static void missing_checkpoints_cb(void* user_data, file_data_t* files, int num_files) {
  ...
  for (int i = 0; i < num_files; i++) {
    ...
    if (!get_checkpoint_from_proof(files[i].data, checkpoint, &slot)) {
      log_error("Failed to get checkpoint from " C4_PS_ZK_PROOF_SSZ ": %l", ctx->periods[i]);
      continue;
    }
