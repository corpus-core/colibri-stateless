  ...
  if (!eth_config.period_master_url) {
    ...
    if (c4_ps_period_index_get_contiguous_from(0, &oldest_period, &newest_period)) {
      for (uint64_t p = period + 1;; p--) {
        bool packed  = c4_ps_file_exists(p, C4_PS_ZK_PROOF_SSZ);
        bool pending = !packed && c4_ps_file_exists(p, C4_PS_ZK_PROOF_G16);
        if (pending)
      ...
      if (c4_ps_file_exists(period, C4_PS_ZK_PROOF_G16)) c4_build_zk_sync_proof_data(period);
      if (c4_ps_file_exists(period + 1, C4_PS_ZK_PROOF_G16)) c4_build_zk_sync_proof_data(period + 1);
    }
