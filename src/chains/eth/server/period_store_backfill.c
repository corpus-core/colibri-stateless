...
void c4_ps_backfill_done(void) {
  ...
  if (eth_config.period_store && latest_hist_period != UINT64_MAX) {
    c4_ps_schedule_verify_all_blocks_for_historical(latest_hist_period);
  }
...
void c4_get_light_client_updates(void* user_data, uint64_t period, uint32_t count, light_client_cb cb) {
  ...
  if (!eth_config.period_store) {
    // Fallback: no cache → fetch all periods directly and return a concatenated result (do not persist).
...
static inline bool read_parent_block(block_t* current, block_t* result) {
  ...
  for (uint64_t i = current->slot; i >= bf_ctx.end_slot; i--) {
    ...
    if ((++scanned_since_yield % 100) == 0) {
      c4_ps_enqueue_backfill(); // schedule continuation on the event loop
      return false;
    }
  }
