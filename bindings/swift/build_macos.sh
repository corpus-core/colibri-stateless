...
# Kopiere src/api/colibri.h für lokale Development (konsistent mit build_ios.sh)
echo "📋 Kopiere src/api/colibri.h für lokales Development..."
mkdir -p "$SWIFT_DIR/Sources/CColibri/include"
cp "$ROOT_DIR/src/api/colibri.h" "$SWIFT_DIR/Sources/CColibri/include/"
...
cat >> "$GENERATED_TESTS_FILE" << 'EOF'
    ...
    /// Serializes fixture tests across `await` points.
    ///
    /// The previous helper acquired a semaphore, spawned an unstructured `Task`,
    /// and released the semaphore in `defer` as soon as that Task was *created*
    /// -- so XCTest / Swift concurrency could run several fixtures at once
    /// against the unsynchronized process-global C caches (`PROVER_CACHE`,
    /// header tags). The gate below only starts the next body after the
    /// previous one has fully finished.
    private final class SerialGate {
        private let lock = NSLock()
        private var tail: Task<Void, Never> = Task {}

        func enqueue<T>(_ work: @escaping () async throws -> T) async throws -> T {
            lock.lock()
            let previous = tail
    ...
    private static let testGate = SerialGate()
    ...
    private func runTestSequentially<T>(_ testName: String, _ testBlock: @escaping () async throws -> T) async throws -> T {
        try await Self.testGate.enqueue(testBlock)
    }
    ...
    private func runSingleTest(testDirectory: URL) async throws -> Bool {
        ...
        if let expectedResult = testJson["expected_result"] {
            ...
            if expectedResult is NSNull {
                matches = result == nil || result is NSNull
                if matches {
                    Self.log("  ✅ Null comparison: both sides are null")
                }
            } else if let expectedDict = expectedResult as? [String: Any],
            ...
            if !matches {
                ...
                } else if expectedResult is NSNull {
