...
#include "colibri.h"
    ...
    % include "stdint.i" % include "various.i"
...
% typemap(jni) bytes_t "jbyteArray" % typemap(jtype) bytes_t "byte[]" % typemap(jstype) bytes_t "byte[]" % typemap(javain) bytes_t "$javainput"
...
% typemap(jni) bytes_t "jbyteArray" % typemap(jtype) bytes_t "byte[]" % typemap(jstype) bytes_t "byte[]" % typemap(javaout) bytes_t { return $jnicall; }
...
% typemap(jni) void * "jlong" % typemap(jtype) void * "long" % typemap(jstype) void * "long" % typemap(javain) void * "$javainput" % typemap(javaout) void* { return $jnicall; }

// Handle prover_t* as an opaque pointer
% typemap(jni) prover_t * "jlong" % typemap(jtype) prover_t * "long" % typemap(jstype) prover_t * "long" % typemap(javain) prover_t * "$javainput" % typemap(javaout) prover_t* { return $jnicall; }
