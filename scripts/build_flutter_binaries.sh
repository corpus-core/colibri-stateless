...
print_usage() {
  echo "Usage: $0 [--android] [--ios] [--windows] [--all]"
  echo ""
...
if [[ $# -eq 0 ]]; then
  ...
  for arg in "$@"; do
    case "$arg" in
      ...
      --all) want_android=true; want_ios=true; want_windows=true ;;
...
build_android() {
  ...
  if [[ -z "$ndk_root" ]]; then
    ...
    if [[ -n "$sdk_root" ]]; then
      if [[ -d "$sdk_root/ndk" ]]; then
        local latest_ndk
        latest_ndk="$(ls -1 "$sdk_root/ndk" 2>/dev/null | sort | tail -n 1 || true)"
        if [[ -n "$latest_ndk" ]]; then
          ndk_root="$sdk_root/ndk/$latest_ndk"
          echo "Detected Android NDK: $ndk_root"
        fi
  ...
  if [[ -z "$ndk_root" ]]; then
    echo "ANDROID_NDK_HOME/ANDROID_NDK is not set. Skipping Android."
    return
  fi
